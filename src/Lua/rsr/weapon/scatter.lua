...
pspractions.A_ScatterRecover = function(player, args)
	...
	if Valid(rsrinfo.scatterFlak) and (rsrinfo.scatterFlak.flags & MF_MISSILE) and (player.cmd.buttons & RSR.GetAttackButton(true)) and not (rsrinfo.lastbuttons & RSR.GetAttackButton(true)) then
		P_ExplodeMissile(rsrinfo.scatterFlak)
	end
end
